...
namespace FastColoredTextBoxNS
{
    ...
    public class MySyntaxHighlighter : SyntaxHighlighter
    {
        ...
        readonly Dictionary<string, string> knownFunctionPatterns = new(StringComparer.Ordinal);

        public MySyntaxHighlighter(FastColoredTextBox currentTb) : base(currentTb)
        {
        }

        public void SetKnownFunctions(IEnumerable<string> functionNames)
        {
            Dictionary<string, SortedSet<string>> functions = new(StringComparer.Ordinal)
            {
                ["Card"] = new(StringComparer.Ordinal),
                ["Duel"] = new(StringComparer.Ordinal),
                ["Group"] = new(StringComparer.Ordinal),
                ["Effect"] = new(StringComparer.Ordinal),
                ["Debug"] = new(StringComparer.Ordinal),
                ["Auxiliary"] = new(StringComparer.Ordinal),
                ["bit"] = new(StringComparer.Ordinal)
            };

            foreach (string functionName in functionNames)
            {
                int separator = functionName.IndexOf('.');
                if (separator <= 0 || separator >= functionName.Length - 1)
                {
                    continue;
                }

                string owner = functionName[..separator];
                string method = functionName[(separator + 1)..];
                if (!functions.ContainsKey(owner) || !IsLuaIdentifier(method))
                {
                    continue;
                }

                _ = functions[owner].Add(Regex.Escape(method));
            }

            knownFunctionPatterns.Clear();
            foreach (KeyValuePair<string, SortedSet<string>> functionGroup in functions)
            {
                if (functionGroup.Value.Count > 0)
                {
                    knownFunctionPatterns[functionGroup.Key] = string.Join("|", functionGroup.Value);
                }
            }
        }
        ...
        public override void LuaSyntaxHighlight(Range range)
        {
            ...
            if (knownFunctionPatterns.Count == 0)
            {
            ...
            ApplyKnownFunctionStyles(range);
        ...
        private void ApplyKnownFunctionStyles(Range range)
        {
            SetOwnerStyle(range, "Card");
            if (knownFunctionPatterns.TryGetValue("Card", out string cardFunctions))
            {
                range.SetStyle(mFunStyle, $@"\b([a-z]{{0,3}}c|a|d):({cardFunctions})\b");
            }

            SetOwnerStyle(range, "Duel");

            SetOwnerStyle(range, "Group");
            if (knownFunctionPatterns.TryGetValue("Group", out string groupFunctions))
            {
                range.SetStyle(mFunStyle, $@"\b[a-z]{{0,3}}g[0-9]{{0,2}}:({groupFunctions})\b");
            }

            SetOwnerStyle(range, "Effect");
            if (knownFunctionPatterns.TryGetValue("Effect", out string effectFunctions))
            {
                range.SetStyle(mFunStyle, $@"\b[a-z]{{0,1}}e[0-9v]{{0,2}}:({effectFunctions})\b");
                range.SetStyle(mFunStyle, $@"\be[0-9v][0-9]{{0,1}}:({effectFunctions})\b");
            }

            SetOwnerStyle(range, "Debug");
            SetOwnerStyle(range, "Auxiliary");
            if (knownFunctionPatterns.TryGetValue("Auxiliary", out string auxiliaryFunctions))
            {
                range.SetStyle(mFunStyle, $@"\baux\.({auxiliaryFunctions})\b");
            }

            SetOwnerStyle(range, "bit");
        }

        private void SetOwnerStyle(Range range, string owner)
        {
            if (knownFunctionPatterns.TryGetValue(owner, out string functions))
            {
                range.SetStyle(mFunStyle, $@"\b{Regex.Escape(owner)}\.({functions})\b");
            }
        }

        private static bool IsLuaIdentifier(string value)
        {
            return Regex.IsMatch(value, @"^[A-Za-z_][A-Za-z_0-9]*$");
        }
